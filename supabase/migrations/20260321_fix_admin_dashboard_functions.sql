...
-- ============================================
-- Purpose: Fix ambiguous column references and add SECURITY DEFINER
-- Created: March 20, 2026
-- ============================================

-- Drop and recreate functions with fixes

-- Function to get table sizes and row counts (unchanged)
...
-- Function to get index usage statistics (unchanged)
...
-- Function to get cache hit ratio (unchanged)
...
-- Function to get database size (unchanged)
...
-- Function to get system metrics (FIXED ambiguous columns)
...
BEGIN
  ...
  -- Get last vacuum and analyze with explicit table qualification
  SELECT COALESCE(date_trunc('minute', pst.last_vacuum), 'Never')::TEXT 
  INTO last_vac
  FROM pg_stat_user_tables pst 
  ORDER BY pst.last_vacuum DESC NULLS LAST 
  LIMIT 1;
  
  SELECT COALESCE(date_trunc('minute', pst.last_analyze), 'Never')::TEXT 
  INTO last_an
  FROM pg_stat_user_tables pst 
  ORDER BY pst.last_analyze DESC NULLS LAST 
  LIMIT 1;
  ...
  SELECT 
    ...
    COALESCE(last_vac, 'Never')::TEXT,
    COALESCE(last_an, 'Never')::TEXT;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Revoke old permissions from public (security)
REVOKE ALL ON FUNCTION get_table_sizes() FROM PUBLIC;
REVOKE ALL ON FUNCTION get_index_stats() FROM PUBLIC;
REVOKE ALL ON FUNCTION get_cache_hit_ratio() FROM PUBLIC;
REVOKE ALL ON FUNCTION get_database_size() FROM PUBLIC;
REVOKE ALL ON FUNCTION get_system_metrics() FROM PUBLIC;
